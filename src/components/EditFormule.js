...
function NewFormule() {
    ...
    const [codFormula, setCodFormula] = useState(formData.cod_formula)
    ...
    const getProductList = async () => {
        ...
        const data = await res.json();
        setNombreProducto(data.find((objeto) => objeto.cod_producto === formData.cod_producto)?.nombre);
        setProductoList(data)
    }

    const getFormuleD = async () => {
        const res = await fetch(`${API}/formule_d`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + jwt
    ...
    const handleChange2 = async (e) => {
        e.preventDefault();
        const res = await fetch(`${API}/formule_edit`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + jwt
